...
def show_map(lat, lon, fire_label, color):
    ...
    folium.Marker([lat, lon], popup=f"{fire_label} at ({lat:.4f}, {lon:.4f})",
    ...
    title_html = f'''<div style="position: absolute; top: 10px; left: 50%; transform: translateX(-50%); z-index: 9999; background-color: {color}; 
                    padding: 12px 28px; border-radius: 10px; color: white; font-weight: bold; font-size: 18px; box-shadow: 0 2px 8px rgba(0,0,0,0.4);">{fire_label}</div>'''
...
if st.button("🔍 Predict Fire Type"):
    # Check if coordinates are within India's boundary box
    if not (8.4 <= lat <= 37.6 and 68.7 <= lon <= 97.25):
        st.error("❌ The entered coordinates are outside India. Prediction is only allowed within Indian territory.")
        proceed = False
    else:
        proceed = True
    ...
    if proceed:
        ...
        # Override if location is near coast and prediction is vegetation
...
# Show results only if prediction exists
if "prediction" in st.session_state:
    ...
    if compare_real:
        ...
        if not nearby_fires.empty:
            st.dataframe(nearby_fires[["acq_date", "latitude", "longitude", "brightness", "bright_t31", "frp", "confidence", "type", "distance_km"]].head(5), use_container_width=True)
        else:
...
st.markdown("<div style='text-align: center; margin-top: 2em; color: gray'>🔥 Powered by MODIS & Streamlit</div>", unsafe_allow_html=True)
